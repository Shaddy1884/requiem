                      ...
                      uint32 len, const uint8 *iv )
{
    ...
    {
        for(i = 0; i < 16; i++)
            output[i] = input[i] ^ tiv[i];

        aes_encrypt(ctx, output, output);
        memcpy(tiv, output, 16);
        input += 16;
        output += 16;
    }
}
...
/* run key generation in reverse, from the given round
   back to round 0 (the original key). */
void aes_reverse_keygen(int round, uint8 *key) {
  while (round > 0) {
    uint8 prevkey[16];
    int i;
    for (i = 4; i < 16; i++) {
      prevkey[i] = key[i] ^ key[i - 4];
    }
    prevkey[3] = key[3] ^ FSb[prevkey[12]];
    prevkey[2] = key[2] ^ FSb[prevkey[15]];
    prevkey[1] = key[1] ^ FSb[prevkey[14]];
    prevkey[0] = key[0] ^ FSb[prevkey[13]] ^ (RCON[round - 1] >> 24);
    
    memcpy(key, prevkey, 16);
    round--;
  }
}
